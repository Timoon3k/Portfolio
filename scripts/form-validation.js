...
const emailValidation = () => {
	...
	if (elementInput.length === 0) {
		emailWarningText.textContent = 'Puste pole!'
		return false
	}

	if (!elementInput.includes('@')) {
		emailWarningText.textContent = 'E-mail powinien zawierać „@”'
		return false
	}

	if (!emailRegExp.test(elementInput)) {
		emailWarningText.textContent = 'Nieprawidłowy email!'
		return false
	}
...
const elementValidation = (element, maxLimit, output) => {
	...
	if (elementInput.length === 0) {
		output.textContent = 'Puste pole !'
		return false
	}

	if (elementInput.length > maxLimit) {
		output.textContent = `Wiadomość jest za długa (max. ${maxLimit} liter)`
		console.log(output)
		return false
	}
